...
function visit(nextFilters) {
    const query = Object.fromEntries(
        Object.entries(nextFilters).filter(([, value]) => value !== '' && value !== false && value !== null && value !== undefined),
    )
...
function scrollToObject(objectId) {
    ...
    document.getElementById(`object-card-${objectId}`)?.scrollIntoView({ behavior: 'smooth', block: 'center' })
}
</script>
    ...
    <div class="mx-auto flex max-w-7xl flex-col gap-4 px-4 py-4 lg:px-6">
        ...
        <main id="main-content" class="grid gap-4 focus:outline-none lg:grid-cols-[minmax(0,65fr)_minmax(22rem,35fr)]" tabindex="-1">
                ...
                <section class={(activeView === 'list' ? 'block' : 'hidden') + ' md:block'} aria-labelledby="catalog-results-heading">
                    ...
                    {:else}
                        <ObjectGrid objects={objects} {isLoading} {selectedObjectId} onHover={(id) => (highlightedObjectId = id)} />
                    {/if}
                </section>
            </main>
    </div>
...
<MobileFilterSheet open={showMobileFilters} filters={activeFilters} {objectTypes} {voivodeships} onClose={() => (showMobileFilters = false)} onApply={(next) => {
 showMobileFilters = false; visit(next) 
}} onClear={() => {
 showMobileFilters = false; clearFilters() 
}} />
